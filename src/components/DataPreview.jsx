  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white overflow-hidden animate-fade-in">
      ...
      <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
        <p className="text-xs text-slate-600 m-0 flex items-center gap-1.5">
          <TableProperties size={13} className="text-indigo-500" />
          <span className="font-medium">Preview {label}</span>
          <span className="text-slate-400">
            — klik baris yang berisi header kolom
          </span>
        </p>
      ...
      {rawRows.length > 10 && (
        <div className="px-4 py-2 bg-slate-50/50 border-t border-slate-200">
          <p className="text-xs text-slate-400 m-0">
            Menampilkan 10 dari {rawRows.length.toLocaleString("id-ID")} baris
          </p>
        </div>
      )}
    </div>
